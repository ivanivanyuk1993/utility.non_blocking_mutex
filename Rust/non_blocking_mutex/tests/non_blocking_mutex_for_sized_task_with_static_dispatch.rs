...
use non_blocking_mutex::sized_task_with_static_dispatch::SizedTaskWithStaticDispatch;
...
fn can_use_Fn() {
    ...
    {
        let non_blocking_mutex = NonBlockingMutexForSizedTaskWithStaticDispatch::new(
        ...
        impl<'last_state_ref, 'unsafe_state_ref, Increment: FnMut(&mut usize) + Send>
            SizedTaskWithStaticDispatch<'unsafe_state_ref, usize>
...
fn can_use_Fn_recursively() {
    ...
    {
        let non_blocking_mutex_arc = Arc::new(NonBlockingMutexForSizedTaskWithStaticDispatch::new(
            max_concurrent_thread_count,
            0,
        ));
...
fn small_state_is_expected() {
    ...
    impl<'state_ref, 'unsafe_state_ref> SizedTaskWithStaticDispatch<'unsafe_state_ref, usize>
        for Task<'state_ref>
    {
        fn run_with_state(self, mut state: MutexGuard<'unsafe_state_ref, usize>) -> () {
            match self.task_args_enum {
...
fn big_state_is_expected() {
    ...
    impl<'state_ref, 'unsafe_state_ref> SizedTaskWithStaticDispatch<'unsafe_state_ref, BigState>
        for Task<'state_ref>
    {
